...
import { Mic2, Lock, Mail, ArrowRight, ArrowLeft, Loader2, Eye, EyeOff } from 'lucide-react';
...
const LoginPage = () => {
  ...
  const [focusedInput, setFocusedInput] = useState(null);  // <--- NUEVO: Para animar el borde al escribir
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');

  // --- CORRECCIÓN BUG PERFORMANCE ---
  // Inyectamos los estilos de animación solo una vez al montar el componente
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      if (formData.email === 'admin@bar.com' && formData.password === '1234') {
        ...
        if (formData.email !== '') {
             setError('Credenciales incorrectas. Prueba: admin@bar.com / 1234');
        } else {
    ...
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        <div style={styles.header}>
          ...
          <h1 style={styles.title}>Acceso Dueños</h1>
          <p style={styles.subtitle}>Gestiona tu Karaoke SaaS</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.inputGroup}>
            ...
            <div style={{
                ...styles.inputWrapper,
                borderColor: focusedInput === 'email' ? '#00f2ff' : '#333', // Feedback visual
              ...
              <input 
                type="email" 
                name="email"
                placeholder="ej: contacto@barmanolo.es"
        ...
        <div style={styles.footer}>
          <p>¿No tienes cuenta? <a href="#" style={styles.link}>Contacta con ventas</a></p>
        </div>
      </div>
    </div>
  );
...
const styles = {
  ...
  iconContainer: {
    ...
    margin: '0 auto 20px auto', border: '1px solid rgba(0, 242, 255, 0.2)',
  },
  ...
  // Wrapper mejorado para soportar iconos y bordes dinámicos
  inputWrapper: {
    position: 'relative',
    display: 'flex',
    alignItems: 'center',
    ...
    borderRadius: '8px',
    border: '1px solid #333', // Borde base
  ...
  input: {
    width: '100%',
    padding: '12px 40px 12px 40px', // Espacio para iconos a ambos lados
    backgroundColor: 'transparent', // Fondo transparente para que se vea el wrapper
    border: 'none',
    color: '#fff',
    fontSize: '16px',
    outline: 'none',
  ...
  link: { color: '#00f2ff', textDecoration: 'none' }
};
